...
function addBountyLabelonIssues(issues) {
  ...
  issues.forEach((issue) => {
    const issueTitleElement = document.querySelector(
      `#issue_${issue.issue_number}_link`
    );
    // console.log('issueTitleElement', issueTitleElement);
    if (
      issueTitleElement &&
      !document.querySelector(`#bounty-label-${issue.issue_number}`)
    ) {
  ...
  if (match) {
    ...
    if (issuePageMetaSection) {
      ...
      if (issueStatusElement && !document.querySelector("#ind-bounty-label")) {
        ...
        issueStatusElement.insertBefore(
          labelContainer,
          issueStatusElement.firstChild
        );
        // TODO: add isClaimed status from the backend
...
function addClaimBountyOnPR(bounties) {
  ...
  if (match) {
    ...
    const prHeaderActions = document.querySelector(
    ...
    if (prHeaderActions) {
      ...
      if (!document.querySelector("#claim-bounty-button")) {
        ...
        prHeaderActions.insertBefore(
          claimBountyButtonContainer,
          prHeaderActions.firstChild
        );
...
function addLoginButton() {
  if (!document.querySelector("#solbounty-login-button")) {
    ...
    ReactDOM.render(<LoginPopup />, loginContainer);
  }
}
