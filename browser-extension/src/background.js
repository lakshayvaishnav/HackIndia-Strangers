...
Browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "GET_BOUNTIES_BY_REPO") {
    axios
      .post(
        `${API_URL}/v1/bounty/by_owner_repo`,
        {
          owner: message.owner,
          repo: message.repo,
        },
  ...
  if (message.type === "GET_BOUNTY_DETAIL") {
    axios
      .post(
        `${API_URL}/v1/bounty/detail/issue_url`,
        {
  ...
  // claim bounty froma a specific issue
  if (message.type === "CLAIM_BOUNTY") {
    axios
      .post(
        `${API_URL}/v1/bounty/attempt`,
        {
  ...
  if (message.type === "GET_PR_DETAIL") {
    axios
      .post(
        `${API_URL}/v1/bounty/detail/pr_url`,
        {
          prUrl: message.prUrl,
        },
        {
  ...
  if (message.type === "SEND_RAW_TRANSACTION") {
    ...
    if (
      typeof serialisedTransaction === "object" &&
      !(serialisedTransaction instanceof Uint8Array)
    ) {
      serialisedTransaction = new Uint8Array(
        Object.values(serialisedTransaction)
      );
    }

    connection
      .sendRawTransaction(serialisedTransaction)
      .then((signature) => {
